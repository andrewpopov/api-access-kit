    ...
    assert(input: unknown): ApiCommandEnvelope<Operation> {
      ...
      if (input.expectedVersion !== undefined) {
        ...
        if (!input.expectedVersion.trim())
          throw new Error(
            "API command expected version must not be blank; omit it entirely if there is no precondition to enforce.",
          );
      }
      return Object.freeze({
        v: 1,
...
export function evaluateApiCommandPrecondition(
  ...
  if (expectedVersion === undefined || expectedVersion === actualVersion) return { allowed: true };
